...
from .utils_Download import collect_downloaded_ids
...
def get_clean_block_text(text):
    if len(text) > 0:
        try:
            ...
            fs_mode = mode(specs['fontsize'][:len(specs['fontsize'])//2])

            inblocks = [blk['content'] for blk in blocks
                        if (blk['fontsize'] >= 0.95*fs_mode and blk['nwords']) > 0]
            text = ' '.join(inblocks)
...
def _push_to_DB(textdata, metadata):
...
def extract_PDFs(
    ...
    for year in years:
        ...
        while new_ids or time.time() < endtime:
            ...
            if not new_ids:
                time.sleep(5)
                print(
                    f"no new data. Will wait {endtime - time.time()} more seconds")
                continue

            processed_ids = downloaded_ids
            paperIds_to_save.append(new_ids)
                    ...
                    for i in pbar:
                        ...
                        if minibatches:
                            ...
                            if TimoutRaised:
                                ...
                                pbar.set_postfix({"status": f"checking valid pages for {len(pending_papers)}" +
