...
def load_and_chunk(paperData: dict,
                   ...
                   max_chunks: int,
                   prefix_len: int = 0):
    win_chunks = chunk_text(paperData["text"], tokenizer,
                            chunk_size=chunk_size,
                            max_chunks=max_chunks,
                            prefix_len=prefix_len)
    texts = win_chunks["chunks"]
    masks = win_chunks["masks"]
    paper_id = paperData["paperId"]
    paperInfo = paperData.copy()
    paperInfo.pop("text")
    payloads = [{**paperInfo, "text": chunk}
                for chunk in win_chunks["chunks"]]
...
class PaperEmbed:
    ...
    def compute_embeddings(self,
                           ...
                           prefix: str = '',
                           suffix: str = ''):
        # text = ' '.join(paperData['pdf_blocks'].split('**BLOCK**'))
        embeddings = []
